...
def read_data(f_rockstar):
    ...
    # MAKE SURE TO SWITCH THE COLUMNS DEPENDING ON WHICH PROPERTY YOU ARE MAPPING TO
    # i.e if you are mapping to angular_momentum, let data[:,0] = ang_momentum
    # and let data[:,6] = m_vir, where 6 is the column number of ang_momentum originally
...
class make_Dataset(Dataset):
    ...
    def __init__(self, name, seed, f_rockstar):
        ...
        self.size   = size
        self.input  = torch.zeros((size, 10), dtype=torch.float) # Each input has a shape of (10,) (flattened)
        ...
        for i in range(size):
            ...
            self.input[i] = halo_data[:,1:11][j]  # load input
            self.output[i] = halo_data[:,0][j]    # Load output
